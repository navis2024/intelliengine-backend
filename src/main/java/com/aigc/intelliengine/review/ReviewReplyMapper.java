package com.aigc.intelliengine.review;

import com.aigc.intelliengine.review.model.entity.ReviewReply;
import com.baomidou.mybatisplus.core.mapper.BaseMapper;
import org.apache.ibatis.annotations.Mapper;
import org.apache.ibatis.annotations.Param;
import org.apache.ibatis.annotations.Select;
import java.util.List;

@Mapper
public interface ReviewReplyMapper extends BaseMapper<ReviewReply> {
    @Select("SELECT * FROM review_reply WHERE comment_id = #{commentId} AND is_deleted = 0 ORDER BY created_at ASC")
    List<ReviewReply> selectByComment(@Param("commentId") Long commentId);
}
